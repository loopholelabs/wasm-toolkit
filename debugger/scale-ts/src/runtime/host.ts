...
export class Host {
    ...
    public static unpackMemoryRef(packed: bigint): [number, number] {
        let ptr = Number((BigInt(packed) >> BigInt(32)) & BigInt(0xffffffff));
        let len = Number((BigInt(packed) & BigInt(0xffffffff)));
        return [ptr, len];
    }
    ...
    public static showContext(c: Context) {
        let req = c.Request;
        console.log("== Context ==");
        console.log("Request method=" + req.Method + ", uri=" + req.URI + ",proto=" + req.Protocol + ", ip=" + req.IP + ", len=" + req.ContentLength);
